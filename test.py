...
def load_and_filter_deadstock(city_name="ВКО"):
    ...
    def extract_tank_and_fuel(tank_value):
        ...
        # Ищем "АИ-80", "АИ-92", "ДТ", "ДТЗ" и пр.
        fuel_match = re.search(
            r'(АИ-80|АИ-92|АИ-95|АИ-98|ДТ|ДТ-Л|ДТ-3-25|ДТ-3-32|СУГ|АИ-95-IMPORT|ДТЗ)',
...
def process_combination_improved(group_key, grouped, oracle_df, city_data):
    ...
    # Проверяем, что данных достаточно (3 месяца почасовых ~ 24*30*3 часов)
    if len(df) < 24 * 30 * 3:
        logging.warning(
            f"Недостаточно данных для OBJECTCODE={object_code}, "
            f"TANK={tank_number}, GASNUM={gasnum_str}"
        )
        return None
    ...
    # Смещаем прогноз к верхним границам
    forecast['yhat'] = forecast['yhat_upper'] * 1.05  # +5% к верхней границе
